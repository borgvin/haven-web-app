...
export const addDesktopStoreWatchers = (store: Store<DesktopAppState,
   Action<DesktopAppState>> & {dispatch: DispatchFunctionType}) => {
    ...
    store.subscribe(connectWatcher(async (isConnected: boolean, wasConnected: boolean) => {
      if (isConnected && !wasConnected) {
        await store.dispatch(initChainData());
